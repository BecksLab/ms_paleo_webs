...
The probability of a trophic interaction ($P_{ij}$) between consumer $i$ and resource $j$ is defined using a Ricker function parameterized by the optimal body-size ratio ($R_{opt}$) and the shape parameter $\gamma$:
...
Although the body size ratio model is parameterised using empirical estimates derived from terrestrial systems (*e.g.,* Serengeti food webs), its applicability to paleo communities may be less direct. To address this, we explicitly evaluated the sensitivity of network structure (see *Parameterisation of the body-size ratio model* section) to variation in key parameters governing interaction probabilities.
...
## Alternative palaeoecological reconstruction approaches and their exclusion

A range of additional approaches have been developed to reconstruct trophic interactions in palaeoecological systems. These methods were not included in the present analysis because they are either conceptually redundant with the frameworks already implemented, or require data types that are not available or not well-resolved for the Cleveland Basin assemblages.
Expert-knowledge and guild-based assembly approaches

Early palaeo food web reconstructions often relied on expert-driven assignment of taxa to trophic guilds, followed by the specification of interactions among guilds (*e.g.,* @roopnarine2006). In these approaches, links are determined by ecological interpretation of functional roles rather than by explicit generative rules applied at the species level. While highly influential, such methods effectively encode expert knowledge as a fixed interaction structure, with limited capacity to propagate uncertainty or explore alternative configurations.

Conceptually, these approaches align closely with feasibility-based models such as PFIM, which also define interactions based on trait compatibility. However, PFIM formalises these rules explicitly and allows stochastic sampling of interaction subsets, thereby enabling replication and statistical comparison across networks. Including both approaches would therefore provide limited additional contrast, as differences would largely reflect implementation rather than underlying ecological assumptions. For this reason, expert-knowledge guild models are not treated as a separate class in this study.

## Phylogenetically informed and trait-evolution models

Some recent approaches incorporate phylogenetic information or trait evolution models to constrain interaction probabilities (e.g., methods inspired by modern ecological network inference such as @fricke2022a). These frameworks assume that trophic interactions are structured by shared evolutionary history or conserved traits, and typically require well-resolved phylogenies and trait datasets.

Such requirements are not met in most deep-time assemblages, including the Cleveland Basin dataset used here. Phylogenetic relationships among fossil taxa are often incomplete, uncertain, or unresolved at the level required to parameterise these models. Moreover, trait data are typically coarse and discretised (*e.g.,* feeding mode, tiering), limiting the applicability of continuous trait-evolution frameworks. As a result, phylogenetically informed reconstruction approaches are not currently feasible for this system and were excluded from the comparative framework.

## Abundance- and co-occurrence–based inference methods

In modern systems, trophic interactions can be inferred from species co-occurrence patterns, abundance data, or statistical association networks (*e.g.,* joint species distribution models, correlation-based networks). These approaches rely on high-resolution ecological sampling and assume that interaction structure can be inferred from patterns of co-variation in space or time.
Fossil datasets generally do not provide the temporal or spatial resolution required to apply such methods robustly. Occurrence data are often time-averaged, spatially aggregated, and subject to preservation and sampling biases, which violate key assumptions of co-occurrence-based inference. Consequently, these approaches are not appropriate for reconstructing trophic interactions in deep-time assemblages and were not considered here.
...
|  | Number of links (L) | ($L = Co \times S^2$), with ($Co \sim U(0.05, 0.25)$) | This study | Connectance randomly drawn per replicate |
| **Niche** | Species richness (S) | Empirical | Input data |  |
|  | Connectance (Co) | ($Co \sim U(0.05, 0.25)$) | This study | Determines niche range distribution |
...
| **Body-size ratio (BSR)** | Body mass ($M_i$) | Uniform within size classes | This study |  |
...
Metrics where $\eta^2$ was notably higher are labelled directly on the bars. The figure highlights that, for the majority of network properties and models, $\eta^2$ values are extremely low (<0.01), confirming that the body size distribution choice has negligible impact on network structure. A few exceptions appear for the ATN model, but these are limited to specific metrics (*e.g.,* number of linear chains).

## Parameterisation of the body-size ratio model

To evaluate the sensitivity of the body-size ratio (BSR) reconstruction model to assumptions about feeding niche structure and interaction filtering, we defined a set of seven parameter configurations spanning biologically plausible perturbations of the interaction probability function.

The BSR model defines trophic link probability as a function of predator–prey body-size ratios and interaction structure is governed by three parameters controlling niche position, niche width, and interaction thresholding.

We explored seven parameter sets (Table Sx), constructed to isolate distinct mechanistic perturbations of this function.

### Table S2. Parameter perturbation classes for the BSR model

| Parameter set | $\alpha$ (optimum ratio) | $\beta$ (niche width) | $\gamma$ (threshold / scaling) | Interpretation |
|--------------|--------------|--------------|--------------|-------------------|
...
### Effects of body-size interaction parameterisation on network structure

Multivariate analysis revealed that parameterisation of the body-size ratio interaction model significantly influenced emergent food-web structure (PERMANOVA, R^2 = 0.227, F = 136.38, p = 0.001). Although statistically highly significant, this effect explains approximately one-fifth of total variation in multivariate network structure, indicating that parameter choice systematically shapes but does not dominate network topology.
...
Given that parameter variation induces structured but low-dimensional shifts in network topology, we retain the baseline parameter set ($\alpha$ = 1.41, $\beta$ = 3.73, $\gamma$ = −1.90; derived from empirical Serengeti estimates) for subsequent analyses.
...
-   $\beta_{S}$: total interaction dissimilarity

-   $\beta_{WN}$: interaction rewiring among shared species

-   $\beta_{OS}$: species turnover–driven interaction change
...
First, simulated network outputs are imported and standardised across models and time steps. A suite of network metrics are extracted and summarised. Descriptive statistics (mean and SD) are calculated for each model–metric combination and exported for supplementary reporting.
