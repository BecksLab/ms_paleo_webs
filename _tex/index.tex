...
\begin{itemize}
\item
  \textbf{Links:} to determine if the number of links are `correct' we
  use \emph{connectance}
\item
  \emph{Shape:} to determine if the `shape' of the network is correct we
  ...
  model is not predicting a similar `shape'). This will allow is to see
  if the models construct tall `pencil' vs flat `pancake' networks
  (Beckerman 2024, pers comms).

  \begin{itemize}
  \tightlist
  \item
    A small (\textless{} 1) number will thus be indicative of a
    `bottom-heavy' network and the opposite for larger numbers
  \end{itemize}
\item
  \textbf{Structure:} to determine if we are capturing some higher level
  of `structure' we are using SVD entropy (Strydom et al., 2021)
\item
  \emph{Motifs:} We can extract the motifs (Milo et al., 2002; Stouffer
  et al., 2007), namely:
